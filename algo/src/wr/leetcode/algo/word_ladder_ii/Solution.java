...
public class Solution {
    public List<String> children ( String word, Set<String> dict) {
        List<String> ret = new LinkedList<>();
        int n = word.length();
        for ( int i = 0; i < n; ++i) {
            for (char ch = 'a'; ch <= 'z'; ++ch ) {
                //if (ch != word.charAt(i) ) {
                    //String val = word.substring(0, i) + ch + word.substring(i+1, n);
    ...
    public List<List<String>> buildLadder( String beginWord, String endWord, Map<String, List<String>> path) {
        List<List<String>> ret = new LinkedList<>();
        if(beginWord.equals(endWord)) {
            List<String> r = new LinkedList<>();
            //BUG: FORGOT TO ADD WORD INTO LIST
            r.add(beginWord);
            ret.add(r);
        } else {
            List<String> parents = path.get(endWord);
            for (String p : parents) {
                List<List<String>> subs = buildLadder(beginWord, p, path);
    ...
    public List<List<String>> findLadders(String beginWord, String endWord, Set<String> wordList) {
        ...
        if(null != beginWord && null != endWord && null != wordList) {
            ...
            //BUG: FORGOT TO ADD ENDWORD
            wordList.add(endWord);

            while(!q.isEmpty()) {
                String w = q.remove();
                if( w.equals(endWord) ) {
                    ret = buildLadder(beginWord, endWord, path);
                    break;
                }
                int d = depth.get(w) + 1;
                List<String> children = children(w, wordList);
                for (String child : children) {
                    if(!depth.containsKey(child)) {
                        depth.put(child, d);
                        q.add(child);
                    } else if (depth.get(child) < d ) {
                        continue;
                    }
                    //BUG: TLE: WE only need to enqueue one time!!!
                    //NOT HERE ENQUEU
                    //q.add(child);

                    List<String> parents = path.getOrDefault(child, new LinkedList<>());
                    parents.add(w);
                    path.put(child, parents);
                }
            }
        }
        return ret;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(sol.findLadders("sand", "acne", dict));
}
/*
    List<List<String>> generatePaths( String root, String curr, Map<String, List<String>> paths){
        List<List<String>> ret = new LinkedList();
        if(curr.equals(root)) {
        ...
        List<String> prevs = paths.getOrDefault(curr, new LinkedList());
        for (String prev : prevs) {
            List<List<String>> sols = generatePaths(root, prev, paths);
    ...
    public List<List<String>> findLadders(String start, String end,
        ...
        while (!queue.isEmpty()) {
            ...
            if (str.equals(end)) {
                ...
                ret = generatePaths(start, end, pathMap);
                break;
            }
            ...
            for (int i = 0; i < str.length(); i++) {
                for (int j = 0; j < 26; j++) {
                    ...
                    if (dict.contains(newStr)) {
                        if (!depthMap.containsKey(newStr)) {
                            // Construct a new node.
                            List<String> path = new LinkedList<>();
                            path.add(str);
                            depthMap.put(newStr, depth +1);
                            pathMap.put(newStr, path);
                            queue.offer(newStr);
                        } else if ( depthMap.get(newStr) == depth +1){
                            pathMap.get(newStr).add(str);
                        }
                    }
                }
            }
